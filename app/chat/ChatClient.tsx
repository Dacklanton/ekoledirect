...
export default function ChatClient() {
  ...
  useEffect(() => { if (matiere) fetchConversations(); }, [matiere]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.expired) { setExpired(true); setSending(false); return; }
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ maxWidth: 600, width: '100%' }}>
          <button className="btn btn-ghost btn-sm" onClick={() => router.push('/dashboard')} style={{ marginBottom: 16 }}>← Retour</button>
    ...
    <div className="flex" style={{ height: '100vh' }}>
      {showSidebar && (
        <div style={{ width: 260, borderRight: '1px solid var(--gray-200)', background: 'white', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: 16, borderBottom: '1px solid var(--gray-200)' }}>
            <button className="btn btn-primary btn-sm btn-full" onClick={() => { setConversationId('new'); setMessages([]); }}>+ Nouvelle conversation</button>
          </div>
          <div style={{ padding: '8px 12px' }}>
            <button className="btn btn-ghost btn-sm" onClick={() => { setMatiere(''); setMessages([]); }}>← Changer de matière</button>
          </div>
          <div style={{ flex: 1, overflow: 'auto', padding: '8px 12px' }}>
            {conversations.map((c: any) => (
              <div key={c.id} onClick={() => { setConversationId(c.id); setMessages([]); }}
                style={{ padding: '10px 12px', borderRadius: 'var(--radius-md)', cursor: 'pointer', marginBottom: 4, background: conversationId === c.id ? 'var(--green-50)' : 'transparent', borderLeft: conversationId === c.id ? '3px solid var(--green-500)' : '3px solid transparent' }}>
          ...
          <div style={{ padding: 12, borderTop: '1px solid var(--gray-200)' }}>
            <button className="btn btn-ghost btn-sm btn-full" onClick={() => router.push('/dashboard')}>📊 Dashboard</button>
          </div>
        </div>
      )}
      ...
      <div className="chat-container" style={{ flex: 1 }}>
        <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--gray-200)', background: 'white', display: 'flex', alignItems: 'center', gap: 12 }}>
          <button className="btn btn-ghost btn-sm" onClick={() => setShowSidebar(!showSidebar)}>{showSidebar ? '◁' : '▷'}</button>
        ...
        <div className="chat-messages">
          {messages.length === 0 && (
            <div className="text-center" style={{ margin: 'auto', maxWidth: 400, padding: 40 }}>
              ...
              <p className="text-muted" style={{ marginTop: 8 }}>Posez n'importe quelle question sur votre cours.</p>
              <div className="flex flex-col gap-sm" style={{ marginTop: 20 }}>
                {getQuickStarts(matiere).map((q, i) => (
                  <button key={i} className="btn btn-secondary btn-sm" onClick={() => { setInput(q); textareaRef.current?.focus(); }}>{q}</button>
                ))}
              </div>
            </div>
          )}
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`chat-bubble ${msg.role} animate-in`}>{msg.content}</div>
          ))}
          ...
          {expired && (
            <div className="card" style={{ maxWidth: 400, margin: '16px auto', textAlign: 'center', background: 'var(--gold-100)', border: '1px solid var(--gold-300)' }}>
              ...
              <p className="text-sm text-muted">Abonnez-vous pour continuer.</p>
            </div>
          )}
        ...
        <div className="chat-input-bar">
          <textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
            placeholder={`Posez votre question en ${matiere}...`} disabled={sending || expired} rows={1} />
...
function getQuickStarts(matiere: string): string[] {
  const map: Record<string, string[]> = {
    'Français': ['Expliquez-moi le discours indirect', 'Comment faire un résumé de texte ?'],
    'Mathématiques': ['Expliquez le théorème de Pythagore', 'Comment résoudre une équation ?'],
    'PCT': ["La loi d'Ohm, c'est quoi ?", 'Expliquez la combustion complète'],
    'SVT': ['Comment fonctionne le système nerveux ?', "C'est quoi un gène ?"],
    'Anglais': ['Help me with present tense', 'How to write a formal letter'],
    'Histoire-Géographie': ["L'indépendance du Dahomey", 'Géographie du Bénin'],
    'Philosophie': ["Qu'est-ce que la conscience ?", 'Liberté et déterminisme'],
  };
  return map[matiere] || ["Sur quel chapitre travaillez-vous ?", "J'ai besoin d'aide avec un exercice"];
}
